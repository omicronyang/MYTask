...
$insertSQL = sprintf("INSERT INTO tk_task (test02, csa_remark1, csa_from_dept, csa_from_user, csa_to_dept, csa_to_user, csa_project, csa_type, csa_text, csa_priority, csa_temp, csa_plan_st, csa_plan_et, csa_plan_hour, csa_status, csa_create_user, csa_last_user, csa_father, csa_root, csa_remark6, csa_project_sub, csa_remark7, csa_remark8, test01, test03, test04) VALUES ($tag $remark %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, '0', '', '', '', '', '')",
...
inner join tk_task_type on tk_task.csa_type=tk_task_type.id 
...
$i=0;
do{
	if($row_Recordset1['csa_father']=="-1"){
$pid= 0;
	} else {
$pid= $row_Recordset1['csa_father'];
	}

$nodename = "[".$row_Recordset1['task_type']."]".$row_Recordset1['csa_text'];
...
$task_arr["task"][$row_Recordset1['tid']]['type'] =  $row_Recordset1['task_type'];
...
$pattaskid = $row_Recordset_task['csa_father'];

mysql_select_db($database_tankdb, $tankdb);
$query_Recordset_pattask = "SELECT * FROM tk_task inner join tk_task_type on tk_task.csa_type=tk_task_type.id WHERE tid = '$pattaskid'";
...
$query_Recordset_subtask = sprintf("SELECT * 
							...
							inner join tk_user on tk_task.csa_to_user=tk_user.uid 
							inner join tk_status on tk_task.csa_status=tk_status.id 
							WHERE tk_task.csa_father = %s ORDER BY csa_last_update DESC", 
...
$sub_task_arr[$row_Recordset_subtask['tid']]['type'] =  $row_Recordset_subtask['task_type'];
...
$wbssum = $row_Recordset_task['csa_root'].">".$row_Recordset_task['tid'].">".$wbsID;
}
...
$task_view_arr["type"]= $row_Recordset_task['task_type'];
...
$query_tktype ="SELECT * FROM tk_task_type ORDER BY task_type_backup ASC";
...
$tktype_arr[$row_tktype['id']]['task_type'] =  $row_tktype['task_type'];
...
$updatetask = sprintf("UPDATE tk_task SET csa_status=%s, csa_remark8=%s, csa_last_user=%s WHERE tid=%s", 
                       GetSQLValueString($status, "text"), 
...
$query_Recordset_task = sprintf("SELECT csa_to_user, csa_text, csa_status, csa_from_user  
FROM tk_task 
WHERE tid = %s", GetSQLValueString($taskid, "int"));
$Recordset_task = mysql_query($query_Recordset_task, $tankdb) or die(mysql_error());
$row_Recordset_task = mysql_fetch_assoc($Recordset_task);
...
$log_time = date("Y-m-d H:i:s");    
  $updateSQL = sprintf("UPDATE tk_task SET csa_status=%s, csa_remark3=%s, csa_last_user=%s WHERE tid=%s", 
                       GetSQLValueString($status, "text"),
...
$wbs = $row_Recordset_task['csa_root'].">".$row_Recordset_task['tid'].">".$wbsid; 
}    
...
$insertSQL = sprintf("INSERT INTO tk_task ($fid_tag $fid_text csa_from_user, csa_to_user, csa_project, csa_type, csa_text, csa_priority, csa_temp, csa_plan_st, csa_plan_et, csa_plan_hour, csa_status, csa_create_user, csa_last_user, csa_father, csa_root, csa_remark6) VALUES ($val_tag $val_text %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
...
$query_Recordset_task = sprintf("SELECT *
							...
							inner join tk_user on tk_task.csa_to_user=tk_user.uid 
							inner join tk_status on tk_task.csa_status=tk_status.id 
								WHERE csa_project = %s AND csa_father = '-1' ORDER BY csa_last_update DESC", GetSQLValueString($prjid, "int"));
...
$sub_task_arr[$row_Recordset_task['tid']]['type'] =  $row_Recordset_task['task_type'];
