...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE tk_task SET csa_from_dept=%s, csa_from_user=%s, csa_to_dept=%s, csa_to_user=%s, csa_type=%s, csa_text=%s, csa_priority=%s, csa_temp=%s, csa_plan_st=%s, csa_plan_et=%s, $plan_hour $csa_remark1 csa_status=%s, csa_last_user=%s, $test02 WHERE tid=%s",
                       ...
                       GetSQLValueString($_POST['csa_status'], "text"),
...
FROM tk_task 
WHERE tid = %s", GetSQLValueString($row_Recordset_task['csa_father'], "int"));
...
mysql_select_db($database_tankdb, $tankdb);
$query_Recordset_type = "SELECT * FROM tk_task_type ORDER BY task_type_backup ASC";
      ...
      <td width="70%"><table width="98%" border="0" cellspacing="0" cellpadding="5" align="right">
          <tr>
            <td ><ul class="breadcrumb">
                ...
                <li>
                  <?php if ($row_Recordset_task['csa_father'] <> -1) { ?>
...
do {  
?>
          <option value="<?php echo $row_Recordset_type['id']?>"<?php if (!(strcmp($row_Recordset_type['id'], $row_Recordset_task['csa_type']))) {echo "selected=\"selected\"";} ?>><?php echo $row_Recordset_type['task_type']?></option>
          <?php
...
?>
        ...
        <?php echo $multilingual_global_hour; ?><span id="plan_hour_msg"></span></td>
          ...
          <tr>
            ...
            <td valign="top"><select name="csa_status" id="csa_status" >
          <?php do {  ?>
          <option value="<?php echo $row_tkstatus['id'] ?>" <?php if (!(strcmp($row_tkstatus['id'], $row_Recordset_task['csa_status']))) {echo "selected=\"selected\"";} ?>><?php echo $row_tkstatus['task_status']?></option>
