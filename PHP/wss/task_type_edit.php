...
$task_type_order = "task_type_backup='0'";
}else{
$task_type_order = sprintf("task_type_backup=%s", GetSQLValueString($_POST['task_type_order'], "text"));
}

if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE tk_task_type SET task_type=%s, $task_type_order WHERE id=%s",
...
mysql_select_db($database_tankdb, $tankdb);
$query_Recordset1 = sprintf("SELECT * FROM tk_task_type WHERE id = %s", GetSQLValueString($colname_Recordset1, "int"));
...
<tbody>
    <tr >
      <td><?php echo $multilingual_tasktype_name; ?>:<br />        
        <input type="text" name="task_status" id="task_status" value="<?php echo htmlentities($row_Recordset1['task_type'], ENT_COMPAT, 'utf-8'); ?>" size="32" class="width-p80" /><span id="csa_text_msg"></span></td>
		<td><?php echo $multilingual_default_order; ?>:<br />        
        <input type="text" name="task_type_order" id="task_type_order" value="<?php echo htmlentities($row_Recordset1['task_type_backup'], ENT_COMPAT, 'utf-8'); ?>" size="32" /><span id="task_type_order_msg"></span></td>
    </tr>
